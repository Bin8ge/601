...
class Auth
{
    ...
    public function getGroups($uid)
    {
        ...
        $user_groups = $model
            ...
            ->field('a.uid,a.group_id,g.id,g.pid,g.name,g.rules')->select();
    ...
    protected function getAuthList($uid, $type)
    {
        ...
        //读取用户组所有权限规则
        $rules = M()->table($this->_config['AUTH_RULE'])->where($map)->field('id,pid,condition,icon,node,name,type')->select();
        ...
        foreach ($rules as $rule) {
            if (!empty($rule['condition'])) { //根据condition进行验证
                ...
                if ($condition) {
                    $authList[$rule['id']] = strtolower($rule['node']);
                }
            } else {
                //只要存在就记录
                $authList[$rule['id']] = strtolower($rule['node']);
