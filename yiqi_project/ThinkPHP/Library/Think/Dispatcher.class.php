    ...
    static public function dispatch()
    {
        ...
        if ('' != $_SERVER['PATH_INFO'] && (!C('URL_ROUTER_ON') || !Route::check())) {   // 检测路由规则 如果没有则按默认规则调度URL
            ...
            if (!defined('BIND_CONTROLLER')) {// 获取控制器
                ...
                // 如果控制器分层模式启用的话 启用2层架构 则增加对文件夹的判断 如果文件夹存在且配置打开的话则进行 多级控制器操作 否则还是单级
                $appController = $_SERVER['DOCUMENT_ROOT']  .str_replace('.','',MODULE_PATH).'Controller/'.ucfirst($paths[0]);
                if (C('CONTROLLER_LEVEL') > 1 && is_dir($appController)) {// 控制器层次
